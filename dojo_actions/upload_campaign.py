...
PROJECT_AUTHOR = os.environ["PROJECT_AUTHOR"]
...
def _fill_variables(text: str):
    text = text.replace("${project_author}", PROJECT_AUTHOR)
...
def create_or_update_campaign(campaign_id: str, campaign: dict):
    ...
    if "expiration_date" in payload:
        payload["expiration_date"] = (
            payload["expiration_date"].isoformat() if payload["end_date"] else None
        )
